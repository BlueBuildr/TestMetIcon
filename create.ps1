#################################################
# HelloID-Conn-Prov-Target-TestMetIcon-Create
# PowerShell V2
#################################################
...
try {
    ...
    if ($actionContext.CorrelationConfiguration.Enabled) {
        ...
        Write-Information "Verifying if a TestMetIcon account exists where $correlationField is: [$correlationValue]"
    ...
    switch ($lifecycleProcess) {
        'CreateAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information 'Creating and correlating TestMetIcon account'
                ...
                Write-Information '[DryRun] Create and correlate TestMetIcon account, will be executed during enforcement'
            }
        ...
        'CorrelateAccount' {
            Write-Information 'Correlating TestMetIcon account'
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not create or correlate TestMetIcon account: [$($actionContext.References.Account)]. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditLogMessage = "Could not create or correlate TestMetIcon account: [$($actionContext.References.Account)]. Error: $($ex.Exception.Message)"
