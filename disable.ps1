##################################################
# HelloID-Conn-Prov-Target-TestMetIcon-Disable
# PowerShell V2
##################################################
...
try {
    ...
    switch ($lifecycleProcess) {
        'DisableAccount' {
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Disabling TestMetIcon account with accountReference: [$($actionContext.References.Account)]"
                ...
                Write-Information "[DryRun] Disable TestMetIcon account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not disable TestMetIcon account: [$($actionContext.References.Account)]. Error: $($errorObj.FriendlyMessage). Action initiated by: [$($actionContext.Origin)]"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditLogMessage = "Could not disable TestMetIcon account: [$($actionContext.References.Account)]. Error: $($_.Exception.Message). Action initiated by: [$($actionContext.Origin)]"
